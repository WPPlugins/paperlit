 ...
 public function render_theme_installed( $theme ) {
  ...
  protected function _upload_theme() {
    ...
    if ( !empty( $_FILES['paperlit-theme-upload'] ) ) {
      ...
      $file_types = wp_check_filetype( basename( sanitize_text_field(esc_attr($_FILES['paperlit-theme-upload']['name']) ) ) );
      $uploaded_type = $file_types['type'];
      // Check if the type is supported. If not, throw an error.
      if ( $uploaded_type == 'application/zip' ) {
        $uploaded = wp_handle_upload($_FILES['paperlit-theme-upload'], array( 'test_form' => false ) );
        $zip = new ZipArchive;
        if ( $zip->open( $uploaded['file'] ) ) {
          if ( $zip->extractTo( PAPERLIT_THEMES_PATH ) ) {
            delete_option( 'paperlit_themes' );
            return true;
          }
        }
  ...
  */
  ...
  public function upload_theme_paperlit() {
    if ( !function_exists( 'wp_handle_upload' ) ) {
      ...
      //foreach dello zips della posiziuone
      $folder     = PAPERLIT_THEMES_DEFAULT_PATH;
      $file_zips = glob($folder."*.*");

      foreach ($file_zips as $file_zip) 
      {
        $name_file  = basename(sanitize_text_field(esc_attr($file_zip)));
     
        $file_types     = wp_check_filetype($name_file);
        $uploaded_type  = $file_types['type'];

        // Check if the type is supported. If not, throw an error.
        if ( $uploaded_type == 'application/zip' ) {
          $uploaded['file'] = $file_zip;
          $zip = new ZipArchive;
          if ( $zip->open( $uploaded['file'] ) ) {
            if ( $zip->extractTo( PAPERLIT_THEMES_PATH ) ) {
              delete_option( 'paperlit_themes' );
          ...
          $zip->close();
          unlink( $uploaded['file'] );
          unlink( $file_zip); // delete the zip 
        }
      }
